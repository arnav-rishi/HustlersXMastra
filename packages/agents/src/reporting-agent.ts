...
const ExecutiveSummaryLlmSchema = z.object({ summary: z.string() });
...
Produce a board-ready executive summary (150 words, FK > 60).
...
// ─── Executive Summary Generation (real agent.generateLegacy() call) ─────────
...
Return JSON: {"summary":"..."}`;
  ...
  try {
    const response = await reportingAgent.generateLegacy<typeof ExecutiveSummaryLlmSchema>(
      [
        { role: "system", content: prompt },
        { role: "user", content: "Generate the executive summary." },
      ],
      { output: ExecutiveSummaryLlmSchema }
    );
    ...
    const parsed = response.object as z.infer<typeof ExecutiveSummaryLlmSchema>;
...
export async function executeReportingAgent(input: ReportingAgentInput): Promise<AnalysisReport> {
  return withSpan(OTEL_SPAN_NAMES.AGENT_REPORTING_GENERATE, {
    ...
    const summaryResult = await generateExecutiveSummary({ contractTitle: input.contractTitle ?? "Contract", jurisdiction: input.jurisdiction, orgId: input.orgId, criticalCount, moderateCount, lowCount, totalClauses, complianceIssues, hitlPending });
