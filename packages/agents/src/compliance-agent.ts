...
const ComplianceLlmSchema = z.object({
  isCompliant: z.boolean(),
  jurisdictionVerified: z.boolean(),
  findings: z.array(z.object({
    regulation: z.string(),
    section: z.string(),
    offendingLanguage: z.string(),
    requiredCorrection: z.string(),
    severity: z.string(),
  })),
});
...
export const complianceAgent: Agent = new Agent({
  ...
  instructions: "Check legal clauses for compliance. Flag if uncertain. Cite specific regulation+section. GDPR/CCPA aware. Escalate to HITL if jurisdictionVerified=false.",
  model: gpt4o,
});

// ─── Compliance Check (real agent.generateLegacy() call) ─────────────────────
...
async function checkCompliance(input: {
  ...
  jurisdiction: string;
  orgId: string;
  jurisdictionRulesContext: string;
  regulatoryDocsContext: string;
}) {
...
Return JSON: {"isCompliant":bool,"jurisdictionVerified":bool,"findings":[{"regulation":"...","section":"...","offendingLanguage":"...","requiredCorrection":"...","severity":"Blocking|Warning|Advisory"}]}`;
  ...
  try {
    const response = await complianceAgent.generateLegacy<typeof ComplianceLlmSchema>(
      [
        { role: "system", content: prompt },
        { role: "user", content: "Check compliance now." },
      ],
      { output: ComplianceLlmSchema }
    );
    ...
    result = response.object as z.infer<typeof ComplianceLlmSchema>;
...
export async function executeComplianceAgent(input: ComplianceAgentInput): Promise<ComplianceResult> {
  return withSpan(OTEL_SPAN_NAMES.LLM_GPT4O_COMPLETION, {
    ...
    const r = await checkCompliance({ clauseType: input.clauseType, clauseIndex: input.clauseIndex, clauseText: input.clauseText, jurisdiction: input.jurisdiction, orgId: input.orgId, jurisdictionRulesContext: rulesCtx, regulatoryDocsContext: docsCtx });
